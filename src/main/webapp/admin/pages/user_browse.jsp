    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header with-border">
          ...
          <a href="user_add.jsp" style="background-color:#2F4F4F;border-color:#D3D3D3;" class="btn btn-info btn-flat pull-right">添加住户</a>
        </div>
        <div class="box-body">
          <div class="box">
            <div class="box-header">
	            <form action="<%=application.getContextPath() %>/app/user/browse">
	              <div class="col-xs-3">
              ...
              <table class="table table-bordered table-hover">
                ...
                <c:forEach items="${users.data }" var="user">
	                <tr>
	                  ...
	                  <td>${user.buyTime }</td>
	                  <td>
	                  	<a href="/manager/app/user/find?user_id=${user.userId}">查看</a>
	                  	<a href="/manager/app/user/edit?user_id=${user.userId}">编辑</a>
        ...
        <div class="box-footer">
          <ul class="pagination pagination-sm no-margin pull-right">
              <c:if test="${users.curPage > 1 }">
              	<li><a href="<%=application.getContextPath() %>/app/user/browse?page=${users.curPage - 1}&keyword=${keyword}">上一页</a></li>
              </c:if>
              
              <c:forEach begin="1" var="pageNum" end="${users.totalPage }" step="1">
              	<li><a href="<%=application.getContextPath() %>/app/user/browse?page=${pageNum}&keyword=${keyword}">${pageNum }</a></li>
              </c:forEach>
              
              <c:if test="${users.curPage < users.totalPage }">
              	<li><a href="<%=application.getContextPath() %>/app/user/browse?page=${users.curPage + 1}&keyword=${keyword}">下一页</a></li>
              </c:if>
          </ul>
        </div>
        <!-- /.box-footer-->
...
<!-- DataTables -->
<script src="<%=application.getContextPath() %>/admin/bower_components/datatables.net/js/jquery.dataTables.min.js"></script>
<script src="<%=application.getContextPath() %>/admin/bower_components/datatables.net-bs/js/dataTables.bootstrap.min.js"></script>
<!-- SlimScroll -->
...
<script src="<%=application.getContextPath() %>/admin/dist/js/user.js"></script>
<script>
