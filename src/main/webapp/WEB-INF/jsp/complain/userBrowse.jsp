...
<!-- about -->
	<div class="services">
		<div class="container">
			<a href="/manager/complain/add.do" style="background-color:#2F4F4F;border-color:#D3D3D3;" class="btn btn-info btn-flat pull-right">添加投诉</a>
	                ...
	                <tr>
	                  <c:forEach items="${complains.data }" var="complain">
		                <tr>
		                  ...
		                  <td>
		                  	<a href="/manager/complain/doUserFind.do?id=${complain.id}">查看</a>
		                  	
	                  		<a href="/manager/complain/userEdit.do?id=${complain.id}">编辑</a>
	                  	
	                  		<a class="complain_del_btn" href="#" data-id="${complain.id }">删除</a>
		...
		<div class="box-footer">
			<ul class="pagination pagination-sm no-margin pull-right">
              <c:if test="${complains.curPage > 1 }">
              	<li><a href="<%=application.getContextPath() %>/complain/doUserBrowse.do?page=${complains.curPage - 1}&keyword=${keyword}">上一页</a></li>
              </c:if>
              
              <c:forEach begin="1" var="pageNum" end="${complains.totalPage }" step="1">
              	<li><a href="<%=application.getContextPath() %>/complain/doUserBrowse.do?page=${pageNum}&keyword=${keyword}">${pageNum }</a></li>
              </c:forEach>
              
              <c:if test="${complains.curPage < complains.totalPage }">
              	<li><a href="<%=application.getContextPath() %>/complain/doUserBrowse.do?page=${complains.curPage + 1}&keyword=${keyword}">下一页</a></li>
              </c:if>
          	</ul>
		</div>
	</div>
...
<!-- here stars scrolling icon -->
	...
	<script type="text/javascript" src="<%=application.getContextPath() %>/static/dist/js/complain.js"></script>
