    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header with-border">
          ...
          <a href="/manager/cost/add.do" style="background-color:#2F4F4F;border-color:#D3D3D3;" class="btn btn-info btn-flat pull-right">添加费用信息</a>
        </div>
        <div class="box-body">
          <div class="box">
            <div class="box-header">
              <form action="<%=application.getContextPath() %>/cost/doBrowse.do">
	              <div class="col-xs-3">
        ...
        <div class="box-footer">
          <ul class="pagination pagination-sm no-margin pull-right">
              <c:if test="${costs.curPage > 1 }">
              	<li><a href="<%=application.getContextPath() %>/cost/doBrowse.do?page=${costs.curPage - 1}&keyword=${keyword}">上一页</a></li>
              </c:if>
              
              <c:forEach begin="1" var="pageNum" end="${costs.totalPage }" step="1">
              	<li><a href="<%=application.getContextPath() %>/cost/doBrowse.do?page=${pageNum}&keyword=${keyword}">${pageNum }</a></li>
              </c:forEach>
              
              <c:if test="${costs.curPage < costs.totalPage }">
              	<li><a href="<%=application.getContextPath() %>/cost/doBrowse.do?page=${costs.curPage + 1}&keyword=${keyword}">下一页</a></li>
              </c:if>
          </ul>
        </div>
        <!-- /.box-footer-->
...
<script src="<%=application.getContextPath() %>/static/dist/js/cost.js"></script>
<!-- page script -->
<script>
  $(function () {
    $('#example1').DataTable()
