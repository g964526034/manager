		...
		<div class="box-footer">
			<ul class="pagination pagination-sm no-margin pull-right">
              <c:if test="${costs.curPage > 1 }">
              	<li><a href="<%=application.getContextPath() %>/cost/doUserBrowse.do?page=${costs.curPage - 1}&keyword=${keyword}">上一页</a></li>
              </c:if>
              
              <c:forEach begin="1" var="pageNum" end="${costs.totalPage }" step="1">
              	<li><a href="<%=application.getContextPath() %>/cost/doUserBrowse.do?page=${pageNum}&keyword=${keyword}">${pageNum }</a></li>
              </c:forEach>
              
              <c:if test="${costs.curPage < costs.totalPage }">
              	<li><a href="<%=application.getContextPath() %>/cost/doUserBrowse.do?page=${costs.curPage + 1}&keyword=${keyword}">下一页</a></li>
              </c:if>
          	</ul>
		</div>
	</div>
