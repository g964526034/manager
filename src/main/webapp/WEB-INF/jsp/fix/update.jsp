...
<!-- about -->
	<div class="services">
		<div class="container">
			<form id="update_fix_form" action="/manager/fix/doUpdate.do" method="post">
				<div class="row">
				  <div class="col-xs-4">
				  	<div hidden="hidden">
				  ...
				  <div class="col-xs-4">
					...
					<div class="form-group">
						<select id="solution" name="solution" class="form-control">
							<option value="0" <c:if test="${fix.solution == 0 }"> selected </c:if>>未解决</option>
							<option value="1" <c:if test="${fix.solution == 1 }"> selected </c:if>>已解决</option>
						</select>
					</div>
				  </div>
				</div>
				<div class="form-group">
				  <label for="fix_message">报修内容如下</label>
				  <textarea id="fix_message" name="fix_message" style="height:300px;" class="form-control" rows="3" placeholder="Enter ..." >${fix.fixMessage }</textarea>
				</div>
				<div class="col-xs-3">
				...
				<div class="col-xs-6" style="text-align:center;">
					...
					<button type="button" id="update_fix_btn" class="btn btn-primary" style="margin-left:5px;">确定</button> 
				</div>
			</form>
		</div>
	</div>
		...
		<div class="container">
			<div class="row">
				<div class="col-xs-3">	
					...
					<input readonly name="real_name" id="real_name" class="form-control" rows="3" type="text" placeholder="标题" value="${fix.admin.realName }">
					<br/>			  
				  
				</div>  
			</div>
			<div class="form-group">
				<label for="response">报修内容如下</label>
				<textarea readonly id="response" name="response" style="height:300px;" class="form-control" rows="3" placeholder="Enter ..." >${fix.response}</textarea>
			</div>
		</div>
	</div>
<!-- //stats -->
