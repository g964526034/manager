    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header with-border">
          ...
          <a href="/manager/announce/add.do" style="background-color:#2F4F4F;border-color:#D3D3D3;" class="btn btn-info btn-flat pull-right">添加公告</a>
        </div>
        <div class="box-body">
          <div class="box">
            <div class="box-header">
              <form action="<%=application.getContextPath() %>/announce/doBrowse.do">
	              <div class="col-xs-3">
				  
				  </div>
				  <div class="col-xs-6">
					  <div class="form-group has-feedback">
						<input type="text" value="${keyword }" name="keyword" class="form-control" placeholder="请输入公告信息">
              ...
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                	<c:forEach items="${announces.data }" var="announce">
		                <tr>
		                  <td>${announce.id }</td>
		                  <td>${announce.admin.realName }</td>
		                  <td>${announce.title}</td>
		                  <td>${announce.createTime }</td>
		                  <td>
		                  	<a href="/manager/announce/doFind.do?id=${announce.id}">查看</a>
		                  	<c:if test="${sessionScope.admin.realName == announce.admin.realName }">
		                  		<a href="/manager/announce/edit.do?id=${announce.id}">编辑</a>
		                  	</c:if>
		                  	<c:if test="${sessionScope.admin.realName == announce.admin.realName }">
		                  		<a class="announce_del_btn" href="#" data-id="${announce.id }">删除</a>
		                  	</c:if>
		                  </td>
		                </tr>
	                </c:forEach>
        ...
        <div class="box-footer">
          <ul class="pagination pagination-sm no-margin pull-right">
              <c:if test="${announces.curPage > 1 }">
              	<li><a href="<%=application.getContextPath() %>/announce/doBrowse.do?page=${announces.curPage - 1}&keyword=${keyword}">上一页</a></li>
              </c:if>
              
              <c:forEach begin="1" var="pageNum" end="${announces.totalPage }" step="1">
              	<li><a href="<%=application.getContextPath() %>/announce/doBrowse.do?page=${pageNum}&keyword=${keyword}">${pageNum }</a></li>
              </c:forEach>
              
              <c:if test="${announces.curPage < announces.totalPage }">
              	<li><a href="<%=application.getContextPath() %>/announce/doBrowse.do?page=${announces.curPage + 1}&keyword=${keyword}">下一页</a></li>
              </c:if>
          </ul>
        </div>
        <!-- /.box-footer-->
...
<script src="<%=application.getContextPath() %>/static/dist/js/announce.js"></script>
<!-- page script -->
<script>
  $(function () {
    $('#example1').DataTable()
