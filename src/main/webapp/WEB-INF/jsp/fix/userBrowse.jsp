	                ...
	                <tr>
	                  <c:forEach items="${fixs.data }" var="fix">
		                <tr>
		                  ...
		                  <td>
		                  	<a href="/manager/fix/doUserFind.do?id=${fix.id}">查看</a>
		                  	
	                  		<a href="/manager/fix/userEdit.do?id=${fix.id}">编辑</a>
	                  	
	                  		<a class="fix_del_btn" href="#" data-id="${fix.id }">删除</a>
		...
		<div class="box-footer">
			<ul class="pagination pagination-sm no-margin pull-right">
              <c:if test="${fixs.curPage > 1 }">
              	<li><a href="<%=application.getContextPath() %>/fix/doUserBrowse.do?page=${fixs.curPage - 1}&keyword=${keyword}">上一页</a></li>
              </c:if>
              
              <c:forEach begin="1" var="pageNum" end="${fixs.totalPage }" step="1">
              	<li><a href="<%=application.getContextPath() %>/fix/doUserBrowse.do?page=${pageNum}&keyword=${keyword}">${pageNum }</a></li>
              </c:forEach>
              
              <c:if test="${fixs.curPage < fixs.totalPage }">
              	<li><a href="<%=application.getContextPath() %>/fix/doUserBrowse.do?page=${fixs.curPage + 1}&keyword=${keyword}">下一页</a></li>
              </c:if>
          	</ul>
		</div>
	</div>
...
<!-- here stars scrolling icon -->
	...
	<script type="text/javascript" src="<%=application.getContextPath() %>/static/dist/js/fix.js"></script>
